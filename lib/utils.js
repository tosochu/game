import { ROOM_STATUS, HUNTER_STATUS, PLAYER_STATUS_IN_ROOM } from './status.js';
import { PROP_R, Props, randomProp } from './prop.js';

export { ROOM_STATUS, PLAYER_STATUS, HUNTER_STATUS, PLAYER_STATUS_IN_ROOM } from './status.js';
export { PROP_R, Props, randomProp } from './prop.js';
...
export const MAP_WIDTH = 10000, MAP_HEIGHT = 8000;
export const PLAYER_R = 20, BLOCK_LENGTH = 200;
export const VIEW_R = 500;

export function checkSegmentCrossSegment(A, B, C, D) {
...
export function checkCircleCrossRectangle({ x, y, r }, { x1, y1, x2, y2 }) {
...
export function checkCircleCrossCircle({ x, y, r }, { x1, y1, r1 }) {
  return Math.hypot(x - x1, y - y1) <= r + r1;
}
export function checkCircleCrossItem(player, item, onlyPosition) {
  var flag = false;
  if (item.type == 'line') {
...
export function generateRoom(length) {
  var items = new Array();
...
export function generateHunters(count) {
  var hunters = {};
...
export function generateProps(count) {
  var props = [];
  for (var i = 0; i < count; i++) {
    props.push({
      type: randomProp(),
      x: RandInt(1, MAP_WIDTH / BLOCK_LENGTH) * BLOCK_LENGTH - BLOCK_LENGTH / 2,
      y: RandInt(1, MAP_HEIGHT / BLOCK_LENGTH) * BLOCK_LENGTH - BLOCK_LENGTH / 2,
    });
  }
  return props;
}

export function checkPointInPolygon({ x, y }, polygon) {
  var result = false;
...
export function getPointToSegmentDistance(x, y, x1, y1, x2, y2) {
  var dx = x2 - x1;
  var dy = y2 - y1;
  var d = dx * dx + dy * dy;
...
export function checkPolygonContainCircle({ x, y, r }, polygon) {
...
export function checkShadowContainCircle(nowx, nowy, circle, x1, y1, x2, y2) {
...
export function getPointToCircleTangent(x0, y0, { x, y, r }) {
...
export function getAllCanSee(room, user) {
  ...
  var prop = [];
  for (var p of room.prop)
    if (checkCircleCrossCircle(player, { x1: p.x, y1: p.y, r1: PROP_R })) prop.push(p);
  ...
  var teammates = {};
  for (var name in room.player)
    if (room.player[name].type == room.player[user].type) teammates[name] = room.player[name];
  return { player: Object.assign(canSeePlayers, teammates), items, prop };
}

export function planPath(x, y, data, { player: players, items }) {
  items = items.filter(x => x.type == 'line');
  var playerNameCanSee = [];
  for (var i in players) {
    if (players[i].type != 'fugitive') continue;
    if (players[i].status == PLAYER_STATUS_IN_ROOM.RUNNING) playerNameCanSee.push(i);
  }
  ...
  if (data.status === HUNTER_STATUS.RANDOM_WALK) {
    ...
    updates.v = Math.hypot(data.target.y - y, data.target.x - x) / (0.15 * 1.2);
  }
  ...
  updates.data = data; if (updates.v > 120) updates.v = 120;
  return updates;
}